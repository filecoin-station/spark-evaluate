...
describe('evaluate', async function () {
  ...
  it('retries on smart contract errors', async () => {
    const round = new RoundData(0n)
    for (let i = 0; i < 5; i++) {
      round.measurements.push({ ...VALID_MEASUREMENT })
    }
    ...
    assert.strictEqual(setScoresCalls.length, 2)
    assert.deepStrictEqual(setScoresCalls[0], setScoresCalls[1])
  })

  it('doesn\'t retry when evaluating the wrong round', async () => {
    const round = new RoundData(0n)
    for (let i = 0; i < 5; i++) {
      round.measurements.push({ ...VALID_MEASUREMENT })
    }
      ...
      async setScores (_, participantAddresses, scores) {
        ...
        Object.assign(err, {
          ...
          info: {
            error: {
              message:
                'failed to estimate gas: message execution failed: exit 33, revert reason: Error(Can only score previous round)'
            }
          }
        })
        throw err
