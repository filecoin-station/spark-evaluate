...
export const buildRetrievalStats = (measurements, telemetryPoint) => {
  ...
  for (const m of measurements) {
    ...
    const { byteLength, ttfb, duration } = m
    ...
    if (ttfb !== undefined && m.status_code === 200) ttfbValues.push(ttfb)
    if (duration !== undefined) durationValues.push(duration)
  }
...
const countUniqueTasks = (measurements) => {
  const getTaskId = (/** @type {import('./preprocess').Measurement} */m) =>
