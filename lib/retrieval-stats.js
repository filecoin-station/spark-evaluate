...
export const recordCommitteeSizes = (committees, point) => {
  ...
  for (const c of committees) {
    ...
    for (const m of c.measurements) {
      ...
      if (m.fraudAssessment === 'OK') data.majoritySize++
    }
  }
  ...
  for (const { subnets, participants, nodes, measurements, majoritySize } of stats.values()) {
    ...
    if (majoritySize > 0 && measurements > 0) {
      majoritySizes.push(Math.floor(100 * majoritySize / measurements))
    }
  }
  ...
  addHistogramToPoint(point, majoritySizes, 'majority_sizes_percents_')
}
