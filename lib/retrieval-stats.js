...
export const buildRetrievalStats = (measurements, telemetryPoint) => {
  ...
  for (const m of measurements) {
    ...
    let oldCount = resultBreakdown[result] ?? 0
    resultBreakdown[result] = oldCount + 1

    const indexerResult = m.indexerResult ?? 'UNDEFINED'
    oldCount = indexerResultBreakdown[indexerResult] ?? 0
    indexerResultBreakdown[indexerResult] = oldCount + 1
