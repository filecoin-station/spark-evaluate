...
const publishRsr = withPgClient(async ({ pgClient, storachaClient, rsrContract }) => {
  ...
  for (const row of rows) {
    for (const [minerId, { successful, total }] of Object.entries(row.providerRetrievalResultStats)) {
      if (!providerRetrievalResultStats.has(minerId)) {
        providerRetrievalResultStats.set(minerId, { successful: 0, total: 0 })
      }
      providerRetrievalResultStats.get(minerId).successful += successful
      providerRetrievalResultStats.get(minerId).total += total
    }
  }
  ...
  const bytes = dagJSON.encode({
    ...
    meta: {
      rounds: rows.map(row => ({
        ...
        measurementBatches: row.measurement_batches.map(cid => ({ '/': cid })),
        details: row.round_details // TODO
      }))
    },
