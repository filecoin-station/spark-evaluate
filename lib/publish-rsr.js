...
import { buildMinerRetrievalStats } from './public-stats.js'
...
export const prepareProviderRetrievalResultStats = withPgClient(async ({ pgClient, round, committees, sparkEvaluateVersion }) => {
  await pgClient.query(`
    INSERT INTO unpublished_rsr_rounds
    (round_index, spark_evaluate_version, measurement_commitments, round_details, provider_retrieval_result_stats)
    VALUES
    ($1, $2, $3, $4, $5)
  `, [
    round.index,
    ...
    round.details,
    buildMinerRetrievalStats(committees)
  ])
})
...
const publishRsr = withPgClient(async ({ pgClient, storachaClient, rsrContract }) => {
  const { rows } = await pgClient.query(`
    ...
    FROM unpublished_rsr_rounds
    WHERE oldest_publishable_round.evaluated_at::date <= evaluated_at AND evaluated_at < oldest_publishable_round.evaluated_at::date + interval '1 day'
    ORDER BY round_index
  `)
  ...
  await pgClient.query(`
    DELETE FROM unpublished_rsr_rounds WHERE round_index <= $1
