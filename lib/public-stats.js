...
const updateIndexerQueryStats = async (pgClient, honestMeasurements) => {
  ...
  const total = tasksWithIndexerResults.size
  const advertisingHttp = tasksWithHttpAdvertisement.size
  debug('Updating public stats - indexer queries: total += %s advertising_http += %s', total, advertisingHttp)
  await pgClient.query(`
    INSERT INTO indexer_query_stats
      (day, total, advertising_http)
    VALUES
      (now(), $1, $2)
    ON CONFLICT(day) DO UPDATE SET
      total = indexer_query_stats.total + $1,
      advertising_http = indexer_query_stats.advertising_http + $2
  `, [
    total,
    advertisingHttp
  ])
