 ...
 * @param {Iterable<Committee>} committees
 */
const updateRetreivalTimes = async (pgClient, committees) => {
  ...
  // conflic should never happen, but in case it does, we average the values
  await pgClient.query(`
    INSERT INTO retreival_times
    ...
    ON CONFLICT(day, miner_id, task_id) DO UPDATE SET
      time_to_first_byte_p50 = (retreival_times.time_to_first_byte_p50 + EXCLUDED.time_to_first_byte_p50) / 2
  `, [
