...
const updateRetrievalTimings = async (pgClient, committees) => {
  ...
  for (const c of committees) {
    ...
    for (const m of c.measurements) {
      if (m.fraudAssessment !== 'OK' || !m.first_byte_at || !m.start_at) continue
    ...
    const ttfbP50 = getValueAtPercentile(ttfbMeasurments, 0.5)
    retrievalTimings.get(minerId).push(Math.ceil(ttfbP50))
  }
