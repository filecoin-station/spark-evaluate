 ...
 * @param {number} stats.successful_http
 */
const updateRetrievalStats = async (pgClient, minerId, { total, successful, successful_http}) => {
  debug('Updating public retrieval stats for miner %s: total += %s successful += %s, sucessful_http += %s', minerId, total, successful, successful_http)
  await pgClient.query(`
    ...
    ON CONFLICT(day, miner_id) DO UPDATE SET
      ...
      sucessful_http = retrieval_stats.successful_http + $4
  `, [
    minerId,
    total,
    successful,
