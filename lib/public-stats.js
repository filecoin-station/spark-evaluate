...
export const updatePublicStats = async ({ createPgClient, committees, allMeasurements, findDealClients }) => {
  ...
  try {
    ...
    await updateTtfbRetreivalStats(pgClient, committees)
  } finally {
    await pgClient.end()
  }
}
...
const updateDailyDealsStats = async (pgClient, committees, findDealClients) => {
  ...
  for (const c of committees) {
    ...
    for (const clientId of clients) {
      ...
      if (evaluation.hasIndexMajority) {
      ...
      if (evaluation.hasRetrievalMajority) {
 ...
 * @param {Iterable<Committee>} committees
 */
const updateTtfbRetreivalStats = async (pgClient, committees) => {
  /** @type {Array<{minerId: string; taskId: string; medianTimeToFirstByte: number}>} */
  const stats = []
  for (const c of committees) {
    if (!c.evaluation || c.evaluation.retrievalResult !== 'OK') continue
    ...
    const medianTimeToFirstByte = getValueAtPercentile(ttfbMeasurments, 0.5)
    stats.push({ minerId, taskId, medianTimeToFirstByte })
  }
  ...
  await pgClient.query(`
    INSERT INTO ttfb_retreival_stats 
    (day, miner_id, task_id, ttfb_median) VALUES 
    (now(), unnest($1::text[]), unnest($2::text[]), unnest($3::int[]))
    ON CONFLICT(day, miner_id, task_id) DO UPDATE SET
      ttfb_median = (ttfb_retreival_stats.ttfb_median + EXCLUDED.ttfb_median) / 2
  `, [
    stats.map(stat => stat.minerId),
    stats.map(stat => stat.taskId),
    stats.map(stat => stat.medianTimeToFirstByte)
  ])
}
