...
export const updateTopMeasurementParticipants = async (pgClient) => {
  const mvTableName = 'top_measurement_participants_yesterday_mv'
  try {
    await pRetry(async () => {
      await pgClient.query(`REFRESH MATERIALIZED VIEW ${mvTableName}`)
    }, {
      retries: 3,
      onFailedAttempt: (error) => {
        console.warn(`Attempt to refresh ${mvTableName} failed: ${error.message}. Retrying...`)
      }
    })
  } catch (err) {
    console.error(`Error refreshing ${mvTableName}`, err)
    Sentry.captureException(err)
  }
}
 ...
 * @param {import('pg').Client} pgClient
 */
export const updateTopMeasurementParticipantsView = async (pgClient) => {
  const mvTableName = 'top_measurement_participants_yesterday_view'
  try {
    await pRetry(async () => {
      await pgClient.query(`REFRESH MATERIALIZED VIEW ${mvTableName}`)
      console.log(`Refreshed ${mvTableName}`)
    }, {
      retries: 3,
      onFailedAttempt: (error) => {
        console.warn(`Attempt to refresh ${mvTableName} failed: ${error.message}. Retrying...`)
