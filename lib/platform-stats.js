...
export const updateStationAndParticipantDetails = async (
  ...
  const participantSubnetsSeen = new Set()
  ...
  for (const m of honestMeasurements) {
    ...
    participantSubnetsSeen.add(getParticipantSubnetKey(m, participantId))
  }
  ...
  debug('Updating recent station details, station_participant_count=%s, participant_subnet_count=%s', statsPerStationParticipant.size, participantSubnetsSeen.size)
  ...
  const participantSubnets = Array.from(participantSubnetsSeen)
...
export const periodicDatabaseRefresh = async (createPgClient) => {
 ...
 * @param {import('pg').Client} pgClient
 */
const updateTopMeasurementParticipants = async (pgClient) => {
  try {
    await pRetry(async () => {
 ...
 * @param {import('pg').Client} pgClient
 */
const aggregateAndCleanupRecentData = async (pgClient) => {
  try {
    ...
    await pgClient.query(`
      ...
      SELECT
        rsdn.day,
        rsdn.accepted_measurement_count,
        rsdn.total_measurement_count,
        rsdn.distinct_active_station_count,
        rsdn.distinct_participant_address_count,
        rps.distinct_inet_group_count
      FROM
        rsdn_summary rsdn
      JOIN
        rps_summary rps ON rsdn.day = rps.day
    ...
    await pgClient.query(`
      DELETE FROM recent_station_details WHERE day <= $1::DATE;
      DELETE FROM recent_participant_subnets WHERE day <= $1::DATE;
    `, [twoDaysAgo]);
 ...
 * @param {import('pg').Client} pgClient
 */
const updateMonthlyActiveStationCount = async (pgClient) => {
  try {
    const lastMonth = new Date();
    lastMonth.setMonth(lastMonth.getMonth() - 1);
    lastMonth.setDate(1); // First day of the previous month
    ...
    if (result.rowCount === 0) {
      ...
      await pgClient.query(`
        ...
        WHERE day >= DATE_TRUNC('month', $1::DATE)
          AND day < DATE_TRUNC('month', $1::DATE) + INTERVAL '1 month'
