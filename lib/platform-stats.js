...
export const periodicDatabaseRefresh = async (createPgClient) => {
  ...
  try {
    const results = await Promise.allSettled([
      updateTopMeasurementParticipants(pgClient),
      aggregateAndCleanupRecentData(pgClient),
      updateMonthlyActiveStationCount(pgClient)
    ])

    results.forEach(result => {
      if (result.status === 'rejected') {
        console.error('Error refreshing database', result.reason)
      }
    })
  } finally {
    await pgClient.end()
