...
export const build = committees => {
  /** @type {Map<string, {total: number, successful: number, successful_http:number}>} */
  ...
  for (const c of committees) {
    ...
    for (const m of c.measurements) {
      ...
      const retrievalStats = providerRetrievalResultStats.get(minerId) ?? { total: 0, successful: 0 , successful_http: 0}
      retrievalStats.total++
      if (m.retrievalResult === 'OK') {retrievalStats.successful++
        if (m.protocol.toLowerCase() === 'http') {
          retrievalStats.successful_http++
        }
      }
