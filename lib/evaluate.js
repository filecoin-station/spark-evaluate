...
export const evaluate = async ({
  ...
  logger.log(
    'EVALUATE ROUND %s: Evaluated %s measurements, accepted %s entries to reward.\n%o',
    roundIndex,
    measurements.length,
  ...
  try {
    recordTelemetry('retrieval_stats_honest', (point) => {
      point.intField('round_index', roundIndex)
      // DISCUSSION POINT: should we include minority measurements in these stats?
      // Right now, we include majority measurements only. As a result,
      // the retrieval-result-breakdown does not show outlier result types
      // like infrequently-encountered HTTP error codes.
