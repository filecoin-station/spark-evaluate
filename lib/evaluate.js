...
export const evaluate = async ({
  ...
  const participants = {}
  let sum = 0n
  let measurementsCount = 0n
  let honestMeasurementsCount = 0n
  ...
  for await (const m of iterateMeasurements()) {
    measurementsCount++
    const fraudAssessment = fraudDetectionStats.fraudAssessments.get(m.id) ?? 'OK'
    fraudAssessments[fraudAssessment] = (fraudAssessments[fraudAssessment] ?? 0) + 1
    if (fraudAssessment === 'OK') {
      honestMeasurementsCount++
...
const getTaskGroupKey = m =>
  encodeURIComponent(`${m.inet_group}${m.cid}${m.provider_address}`)
...
export const runFraudDetection = async (roundIndex, sparkRoundDetails) => {
  const fraudAssessments = new Map()
  const taskGroupKeys = new Set()
  ...
  for await (const m of iterateMeasurements()) {
    ...
    if (!isValidTask) {
      fraudAssessments.set(m.id, 'INVALID_TASK')
      debug(
  ...
  for await (const m of iterateMeasurements()) {
    const taskGroup = getTaskGroupKey(m)
    const h = await getHash(m)
    if (taskGroupWinners.get(taskGroup) !== h) {
      fraudAssessments.set(m.id, 'DUP_INET_GROUP')
      debug(
        'FRAUD ASSESSMENT for round=%s client=%s cid=%s',
        roundIndex,
        m.participantAddress,
