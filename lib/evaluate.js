...
export const evaluate = async ({
  ...
  await runFraudDetection(roundIndex, measurements, sparkRoundDetails, recordTelemetry, logger)
 ...
 */
export const runFraudDetection = async (roundIndex, measurements, sparkRoundDetails, recordTelemetry, logger = console) => {
  ...
  const taskWithKeys = await Promise.all(sparkRoundDetails.retrievalTasks.map(async (task) => {
    ...
    const key = BigInt('0x' + Buffer.from(hash).toString('hex'))
    return { ...task, key }
  }))

  const seeker = new closest.Seeker([...taskWithKeys], (targetKey, t) => t.key ^ targetKey)

  /** @type {Map<string, import('./typings.js').RetrievalTask[]>} */
  const stationTasks = new Map()
  for (const m of measurements) {
    if (stationTasks.has(m.stationId)) continue
    const hash = await crypto.subtle.digest('sha-256', Buffer.from(m.stationId))
    const stationKey = BigInt('0x' + Buffer.from(hash).toString('hex'))

    /** @typedef {import('./typings.js').RetrievalTask & {key: bigint}} TaskWithKey */
    ...
    // }
    // // TODO: use k-closest-element algorithm for better performance
    // const tasks = [...taskWithKeys]
    // tasks.sort(comparator)
    // tasks.splice(sparkRoundDetails.maxTasksPerNode)

    const tasks = seeker.wHeap(stationKey, sparkRoundDetails.maxTasksPerNode)

    // tasks.sort()
    // tasks2.sort()
    // const s1 = JSON.stringify(tasks.map(({ cid, minerId }) => ({ cid, minerId })))
    // const s2 = JSON.stringify(tasks2.map(({ cid, minerId }) => ({ cid, minerId })))

    // console.log('tasks - our: %o', s1)
    // console.log('tasks - lib: %o', s2)

    // console.log('SAME?', s1 === s2)

    stationTasks.set(m.stationId, tasks)
  }
  logger.log(
  ...
  // Experiment: what if each station executes exactly the tasks that are assigned to it?
  // What will the distribution of committees look like?
  /** @type {Map<string, { inet_group: string, participantAddress: string }>} */
  ...
  for (const [stationId, tasks] of stationTasks.entries()) {
    /* eslint-disable-next-line camelcase */
    const { participantAddress, inet_group } = stations.get(stationId)
    for (const t of tasks) {
      dummyMeasurements.push({
        cid: t.cid,
        minerId: t.minerId,
        stationId,
        participantAddress,
  ...
  recordTelemetry('committees_nextgen', (point) => {
    recordCommitteeSizes(dummyMeasurements, point)
  })
  console.log('MEASUREMENTS REAL=%s DUMMY=%s', measurements.length, dummyMeasurements.length)
  ...
  for (const m of measurements) {
    ...
    const isValidTaskForNode = stationTasks.get(m.stationId).some(
      t => t.cid === m.cid && t.minerId === m.minerId
    )
    if (!isValidTaskForNode) {
      m.fraudAssessment = 'TASK_NOT_ALLOWED'
    }
  }
  ...
  const getHash = async (/** @type {import('./preprocess.js').Measurement} */ m) => {
