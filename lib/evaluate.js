 ...
 * @param {import('cancel-stuck-transactions').CancelStuckTransactions} args.cancelStuckTransactions
 */
export const evaluate = async ({
  ...
  for (const [bucketIndex, bucket] of Object.entries(buckets)) {
    try {
      tx = await ieContractWithSigner.setScores(
        roundIndex,
        bucket.participants,
        bucket.scores
      )
  ...
  ;(async () => {
    await tx.wait()
    cancelStuckTransactions.successful(tx)
  })().catch(console.error)
