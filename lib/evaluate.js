 ...
 * @param {(round: import('./round.js').RoundData, committees: Iterable<import('./committee.js').Committee>) => Promise<void>} args.prepareAcceptedRetrievalTaskMeasurementsCommitment
 */
export const evaluate = async ({
  ...
  createPgClient,
  logger,
  prepareAcceptedRetrievalTaskMeasurementsCommitment
}) => {
  ...
  try {
    await prepareAcceptedRetrievalTaskMeasurementsCommitment(round, committees)
  } catch (err) {
    console.error('Cannot execute `prepareAcceptedRetrievalTaskMeasurementsCommitment` hook.', err)
    ignoredErrors.push(err)
    Sentry.captureException(err)
  }
