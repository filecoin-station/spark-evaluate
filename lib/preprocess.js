...
export const fetchMeasurementsViaClient = async (web3Storage, cid) => {
  // TODO: Test if the old client works with the new API, or whether there
  // is a different one we should use
...
export const fetchMeasurementsViaGateway = async (cid) => {
  // TODO: Can we make it use the old URL scheme?
  const res = await fetch(`https://${encodeURIComponent(cid)}.ipfs.w3s.link/`)
  if (!res.ok) {
