...
const ROUND_LENGTH_MS = 20 * 60 * 1000 // 20 minutes
...
export const cancelStuckTxs = async ({ walletDelegatedAddress, signer }) => {
  ...
  const txsToCancel = messages.filter(m => {
    return new Date(m.createTimestamp * 1000) <
      new Date(Date.now() - ROUND_LENGTH_MS)
  })
  ...
  console.log('Going to revoke:')
  for (const tx of txsToCancel) {
    console.log('-', tx.cid, new Date(tx.createTimestamp * 1000).toISOString())
  }
